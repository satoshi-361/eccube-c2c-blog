        ...
        if ($form->isSubmitted() && $form->isValid()) {
            switch ($request->get('mode')) {
                case 'confirm':
                    log_info('会員登録確認開始');
                    log_info('会員登録確認完了');
                    $plan_poster = $form['plan_poster']->getData();
                    $this->session->set('plan_poster', $plan_poster);
                    
                    $files = $form['authorize_doc']->getData();
                    $authorize_doc = [];

                    foreach($files as $file) {
                        $extension = $file->getClientOriginalExtension();
                        $filename = date('mdHis').$file->getClientOriginalName();
                        $file->move($this->eccubeConfig['eccube_save_image_dir'], $filename);
                        $authorize_doc[] = $filename;
                    }
                    
                    $this->session->set('authorize_doc', implode(',', $authorize_doc));
                ...
                case 'complete':
                    ...
                    if ($this->session->has('authorize_doc'))
                        $Customer->setAuthorizeDoc($this->session->get('authorize_doc'));
                    ...
                    $this->session->remove('plan_poster');
                    $this->session->remove('authorize_doc');
