...
class ShoppingController extends AbstractShoppingController
{
     ...
     */
    public function index(PurchaseFlow $cartPurchaseFlow, $stripe_error = '')
    {
        // ログイン状態のチェック.
        ...
        $this->session->set('payment_total', $Order->getPaymentTotal());
     ...
     *
     * @Route("/shopping/confirm", name="shopping_confirm", methods={"POST", "GET"})
     ...
     *
     * @Route("/shopping/checkout/{stripe_is_successful}", name="shopping_checkout", methods={"POST", "GET"})
     * @Template("Shopping/confirm.twig")
     */
    public function checkout(Request $request, $stripe_is_successful = false)
    {
        // ログイン状態のチェック.
        if ($this->orderHelper->isLoginRequired()) {
        ...
        if (($form->isSubmitted() && $form->isValid()) || $stripe_is_successful) {
     ...
     * 購入エラー画面.
     *
     * @Route("/shopping/stripe/checkout", name="shopping_stripe_checkout", methods={"POST"})
     */
    public function stripeCheckout(Request $request)
    {
        ...
        $stripe = new \Stripe\StripeClient(
          'sk_test_51L7uTiAA374F9rqHy25TVj80WoLyvQjLfPfG8I4enaNiVxJgSrIUIWSbeBB5euyfV6bhB9q7hA9qYxuaW9elxwn6007dJtYFHl'
        );

        try {
            $stripe->charges->create([
              'amount' => $this->session->get('payment_total'),
              'currency' => 'jpy',
              'source' => $stripeToken,
              'description' => $stripeEmail,
            ]);
        } catch (\Stripe\Exception\CardException $e) {
            return $this->redirectToRoute('shopping', $e->getError()->message . '\n');
        ...
        return $this->redirectToRoute('shopping_checkout', ['stripe_is_successful' => true]);
    }
}
