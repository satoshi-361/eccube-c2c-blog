...
class MypageController extends AbstractController
{
     ...
     * お気に入り商品を表示する.
     *
     * @Route("/mypage/sale", name="mypage_sale", methods={"GET"})
     * @Template("Mypage/sale.twig")
     */
    public function sale(Request $request, PaginatorInterface $paginator)
    {
        $qb = $this->orderItemRepository->createQueryBuilder('oi');

        $Products = [];
        $Blogs = $this->getUser()->getBlogs();
        foreach($this->getUser()->getBlogs() as $Blog)
            $Products[] = $Blog->getProduct();

        $qb
            ->innerJoin('oi.Product', 'p')
            ->innerJoin('oi.Order', 'o')
            ->where($qb->expr()->in('p', ':Products'))
            ->andWhere('o.order_date >= :start0')
            ->andWhere('o.order_date <= :today')
            ->setParameter('Products', $Products)
            ->setParameter('start0', date('Y-m-01 00:00'))
