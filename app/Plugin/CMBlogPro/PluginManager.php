...
class PluginManager extends AbstractPluginManager
{
    ...
    const PAGEEDITNAME = '簡単ブログPro編集ページ';
    const PAGEEDITFILENAME = "blog/edit";
    const PAGEEDITURL = "cm_blog_pro_edit";
    
    const PAGEEDITNAME1 = '簡単ブログPro作成ページ';
    const PAGEEDITFILENAME1 = "blog/edit";
    const PAGEEDITURL1 = "cm_blog_pro_create";
...
___SQL;
    ...
    public function uninstall(array $meta, ContainerInterface $container)
    {
        ...
        try {
            ...
            $this->removePageEdit($em, $container);
            $em->getConnection()->commit();
     ...
     * @param ContainerInterface $container
     */
    protected function removePageEdit(
        EntityManagerInterface $em,
        ContainerInterface $container)
    {
        $entityManager = $container->get('doctrine')->getManager();
        $PageEdit = $entityManager->getRepository(Page::class)->findOneBy(
            [
                'name'       => self::PAGEEDITNAME,
                'file_name'  => self::PAGEEDITFILENAME,
                'url'        => self::PAGEEDITURL,
                'edit_type'  => PAGE::EDIT_TYPE_DEFAULT
            ]
        );
        if ($PageEdit) {
            ...
            $Layout = $entityManager->getRepository('Eccube\Entity\Layout')->find(2);
            $PageLayout = $entityManager->getRepository(PageLayout::class)->findOneBy(['Page' => $PageEdit, 'Layout' => $Layout]); // Blockの削除 $entityManager = $container->get('doctrine')->getManager(); 
            
            $this->removeObject($em, $PageLayout);
            $this->removeObject($em, $PageEdit);
        }

        
        $PageEdit1 = $entityManager->getRepository(Page::class)->findOneBy(
            [
                'name'       => self::PAGEEDITNAME1,
                'file_name'  => self::PAGEEDITFILENAME1,
                'url'        => self::PAGEEDITURL1,
                'edit_type'  => PAGE::EDIT_TYPE_DEFAULT
            ]
        );
        if ($PageEdit1) {
            ...
            $Layout = $entityManager->getRepository('Eccube\Entity\Layout')->find(2);
            $PageLayout = $entityManager->getRepository(PageLayout::class)->findOneBy(['Page' => $PageEdit1, 'Layout' => $Layout]); // Blockの削除 $entityManager = $container->get('doctrine')->getManager(); 
            
            $this->removeObject($em, $PageLayout);
            $this->removeObject($em, $PageEdit1);
        }
    }
        ...
        ContainerInterface $container)
    {
        ...
        if ($Page) {
            ...
            $this->createTemplate(
                ...
                '/', self::PAGEEDITFILENAME);
            return $Page;
        }
        // 新しいデータを作成する
        $Page = new Page();
        $Page
            ->setUrl(self::PAGEEDITURL)
            ->setFileName(self::PAGEEDITFILENAME)
            ->setName(self::PAGEEDITNAME)
     ...
     * @param ContainerInterface $container
     */
    protected function initEditPage1(
        EntityManagerInterface $em,
        ContainerInterface $container)
    {
        $entityManager = $container->get('doctrine')->getManager();
        ...
        if ($Page) {
            ...
            $this->createTemplate(
                $container->getParameter('eccube_theme_front_dir'),
                '/', self::PAGEEDITFILENAME1);
            return $Page;
        }
        // 新しいデータを作成する
        $Page = new Page();
        $Page
            ->setUrl(self::PAGEEDITURL1)
            ->setFileName(self::PAGEEDITFILENAME1)
            ->setName(self::PAGEEDITNAME1)
            ->setEditType(PAGE::EDIT_TYPE_DEFAULT);
        $this->createTemplate(
            $container->getParameter('eccube_theme_front_dir'),
            '/', self::PAGEEDITFILENAME1);
    ...
    public function enable(array $meta, ContainerInterface $container)
    {
        ...
        try {
            ...
            $EditPage = $this->initEditPage($em, $container);
            $EditPageLayout = $this->initPageLayout($em, $container, $EditPage);
            $EditPage1 = $this->initEditPage1($em, $container);
            $EditPageLayout1 = $this->initPageLayout($em, $container, $EditPage1);
