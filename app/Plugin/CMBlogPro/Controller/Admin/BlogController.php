    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        if ('POST' === $request->getMethod()) { 
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (is_null($Blog->getProduct())) {
                    $Product = new Product();
                    $ProductClass = new ProductClass();

                    if ( $form['status']->getData()->getId() === \Plugin\CMBlogPro\Entity\BlogStatus::DISPLAY_SHOW ) { 
                    ...
                    $ProductClass
                        ...
                        ->setStockUnlimited(true)
                        ->setProduct($Product)
                        ->setSaleType($this->saleTypeRepository->find(\Eccube\Entity\Master\SaleType::SALE_TYPE_NORMAL))
                        ->setPrice02($form['price02']->getData());
                    ...
                    if ( $Blog->getStatus()->getId() === \Plugin\CMBlogPro\Entity\BlogStatus::DISPLAY_SHOW ) {
                        $ProductStatus = $this->productStatusRepository->find(ProductStatus::DISPLAY_SHOW);
                    } else {
                        $ProductStatus = $this->productStatusRepository->find(ProductStatus::DISPLAY_HIDE);
                    }
                    ...
                    $ProductClass = $Product->getProductClasses()->last();
                    $ProductStock = $ProductClass->getProductStock();
                    $ProductClass->setPrice02($form['price02']->getData());
                }

                // 在庫情報を作成
                if (!$form['stock_unlimited']->getData()) {
                    $ProductStock->setStock($form['stock']->getData());
                    $ProductClass->setStockUnlimited(false);
                    $ProductClass->setStock($form['stock']->getData());
                } else {
                    // 在庫無制限時はnullを設定
                ...
                $ProductClass->setSaleLimit($form['sale_limit']->getData());
                ...
                $Product->setName($form['title']->getData());
                $Product->setDescriptionDetail(strip_tags($form['body']->getData()));
                ...
                foreach($Categories as $Category) {
                    $tempCategories[] = $this->defaultCategoryRepository->findOneBy([ 'name' => $Category->getName() ]);
                }
                ...
                if ( empty($Product->getProductImage()) || $Product->getMainListImage() != $Blog->getMainListImage() ) {
                    ...
                    $ProductImage
                        ->setFileName($Blog->getMainListImage())
                        ->setProduct($Product)
                        ->setSortNo(1);
